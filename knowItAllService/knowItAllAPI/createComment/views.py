...
def createComment(request):
    ...
    if not anonymous.isdigit() or not (0 <= int(anonymous) <= 1):
        return JsonResponse(createComment_400_ALL, status=400)
    anonymous = int(anonymous)
    anonymous = True if anonymous == 1 else False

    # Check if all parameters provided
    if any(var is None for var in [username, pollText, comment]):
        return JsonResponse(createComment_400_ALL, status=400, safe=False)
